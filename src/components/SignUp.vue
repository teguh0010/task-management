...
import { auth, storage, db } from '../db'
...
const signUpWithGoogle = async () => {
  ...
  try {
    ...
    if (docSnap.exists()) {
      // User already exists, so you might want to notify the user or handle as needed
      toast.add({ severity: 'warn',  detail: 'Email already exists, please login.', life: 3000 });
    } else {
      // User does not exist, proceed to add them to Firestore
    ...
    // Handling specific Firebase Auth error codes
    if (err.code === 'auth/email-already-in-use') {
      toast.add({ severity: 'error', summary: 'Signup Error', detail: 'Email already in use. Please login.', life: 3000 });
    } else {
      toast.add({ severity: 'error', summary: 'Signup Error', detail: err.message, life: 3000 });
    }
  }
};

const show = () => {
    toast.add({severity:'info',  detail: 'Here is a detail message', life: 2000});
};
</script>
